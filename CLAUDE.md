...
All tooling lives in `scripts/` (Node >= 20):

```bash
cd scripts
npm ci                 # install pinned dependencies
npm run validate:data  # structural validation of cards + categories — run first
npm run build:docs     # regenerate docs/ (deterministic; fails closed on invalid data)
npm run check:docs     # verify docs/ matches the data — no writes, fails on drift
npm test               # focused generator/validator tests (node --test)
npm audit              # must report 0 vulnerabilities
```

CI (`.github/workflows/ci.yml`) runs the same gates on every push/PR to `main`.
...
Data-first atlas: each tool's YAML frontmatter in `research/<category>/<tool>.md`
is the single source of truth; `data/categories/*.yaml` carries per-category
metadata; `docs/` is a deterministic rendering of that data produced by
`scripts/build-docs.mjs`. Edit the data, then rebuild — never edit `docs/`
pages by hand (except `docs/mba/` and `docs/beads/`, which are third-party
workflow references, not generated).

## Conventions & Patterns

- Content/data are CC0-1.0; project software is Apache-2.0 — scope in
  `LICENSING.md` (with `THIRD_PARTY_NOTICES.md` and `TRADEMARKS.md`).
  Keep new scripts under Apache-2.0 with SPDX headers.
- Managed instruction files are LF-only via `.gitattributes`; do not edit the
  generated `BEADS INTEGRATION` blocks or the `MBA RULES` block by hand.
- `.beads/`, `.mba/`, `.mba-work/`, `.opencode/`, and `opencode.json` are
  machine-local and must never be tracked.
