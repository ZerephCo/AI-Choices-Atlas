...
|---|---|---|---|---|---|---|---|---|
| [claude-mem](#claude-mem) | 4.1 Project / task memory | Apache-2.0 | 87,904 | Prominent | 🟢 Active | ✅ | docs-only | Persistent session-memory plugin — captures what an agent does, AI-compresses it into observations, and injects relevant context into future sessions. |
| [codegraph](#codegraph) | 4.1 Project / task memory | MIT | 61,125 | Prominent | 🟢 Active | ✅ | docs-only | Pre-indexed code knowledge graph served to agents via MCP — auto-syncs on code changes; fewer tokens, fewer tool calls, 100% local. |
| [MemPalace](#mempalace) | 4.1 Project / task memory | MIT | 57,490 | Prominent | 🟢 Active | ✅ | docs-only | Local memory-palace system (method of loci) — verbatim storage in wings/rooms/closets/drawers, a symbolic AAAK index (~170-token startup), temporal KG; fully offline. |
| [codebase-memory-mcp](#codebase-memory-mcp) | 4.1 Project / task memory | MIT | 33,032 | Prominent | 🟢 Active | ✅ | docs-only | Code-intelligence MCP server — indexes a codebase into a persistent knowledge graph (functions/classes/call-chains); 158 languages, sub-ms, local, no keys. |
| [Taskmaster](#taskmaster) | 4.1 Project / task memory | MIT + Commons Clause / NOASSERTION | 27,870 | Prominent | 🟢 Active | 🟡 partial | docs-only | AI-powered PRD-driven task manager — decomposes a requirements doc into a dependency-aware task list you drive from your editor (Cursor/Claude Code). |
| [Beads](#beads) | 4.1 Project / task memory | MIT | 25,436 | Prominent | 🟢 Active | ✅ | run | A Dolt-backed distributed graph issue tracker giving coding agents persistent, dependency-aware task memory — a memory upgrade for your coding agent. |
| [Roo Code Memory Bank](#roo-code-memory-bank) | 4.1 Project / task memory | Apache-2.0 | 1,678 | Emerging | ⚠️ Caution | ✅ | docs-only | Markdown memory bank for Roo Code (VS Code) — structured .md files that keep project context across sessions; no DB, convention-driven. |
| [magic-context](#magic-context) | 4.1 Project / task memory | MIT | 1,404 | Emerging | 🟢 Active | ✅ | docs-only | The hippocampus for coding agents (CortexKit) — self-managing session memory with no compaction pauses and no forgetting across restarts. |
| [Mem0](#mem0) | 4.2 Personal memory | Apache-2.0 | 61,238 | Prominent | 🟢 Active | 🟡 partial | inspected | Universal memory layer giving agents user/session/agent-level recall + personalization; add-only fact extraction with multi-signal retrieval. |
| [Supermemory](#supermemory) | 4.2 Personal memory | MIT | 28,490 | Prominent | 🟢 Active | ✅ | docs-only | The Memory API for the AI era — a fast, scalable memory+context engine + app (RAG + memory + profiles + connectors), runnable fully local/air-gapped. |
| [Memori](#memori) | 4.2 Personal memory | open-source (claimed) / NOASSERTION | 15,633 | Prominent | 🟢 Active | 🟡 partial | docs-only | SQL-native memory engine — persistent agent memory in standard SQL (SQLite/Postgres/MySQL), no vector DB, portable and queryable. |
| [OpenMemory](#openmemory) | 4.2 Personal memory | Apache-2.0 | 4,355 | Established | 🟢 Active | ✅ | docs-only | Cognitive memory engine for LLMs — local, self-hosted; hierarchical decomposition + temporal awareness; native MCP; migrates from Mem0/Zep/Supermemory. |
| [Graphiti](#graphiti) | 4.3 Organizational / semantic memory | Apache-2.0 | 28,940 | Prominent | 🟢 Active | ☁️ hosted | docs-only | Framework for temporal knowledge graphs for agents — tracks when facts became and ceased to be true, with hybrid retrieval and full provenance. |
| [Cognee](#cognee) | 4.3 Organizational / semantic memory | Apache-2.0 | 28,535 | Prominent | 🟢 Active | ✅ | docs-only | Open-source graph-native memory platform — an ECL (Extract→Cognify→Load) pipeline that turns data into a self-improving knowledge graph + vectors. |
| [GBrain](#gbrain) | 4.3 Organizational / semantic memory | MIT | 26,622 | Prominent | 🟢 Active | 🟡 partial | docs-only | Self-hosted institutional memory — a git-versioned markdown brain repo + Postgres/pgvector index with a self-wiring typed-edge graph and cited answers. |
| [memU](#memu) | 4.3 Organizational / semantic memory | NOASSERTION | 14,048 | Prominent | 🟢 Active | 🟡 partial | docs-only | Cross-agent personal memory for 24/7 proactive agents — a self-organizing markdown file system with intent capture and ~10x token reduction; embeddings-only. |
| [semble](#semble) | 4.3 Organizational / semantic memory | MIT | 5,654 | Established | 🟢 Active | ✅ | docs-only | CPU-only code search for agents — static Model2Vec embeddings + BM25 + RRF; ~98% fewer tokens than grep+read; MCP; no GPU/keys. |
| [Cipher](#cipher) | 4.3 Organizational / semantic memory | NOASSERTION | 4,925 | Established | 🟢 Active | 🟡 partial | docs-only | Open-source memory layer for coding agents — dual System-1/System-2 memory, cross-IDE via MCP, team-shareable; renamed to ByteRover CLI (brv). |
| [Zep](#zep) | 4.3 Organizational / semantic memory | Apache-2.0 | 4,766 | Established | ⚠️ Caution | ☁️ hosted | docs-only | Enterprise agent-memory platform — temporal knowledge-graph memory (built on Graphiti) with compliance; the managed counterpart to OSS Graphiti. |
| [LangMem](#langmem) | 4.3 Organizational / semantic memory | MIT | 1,568 | Established | 🟢 Active | ☁️ hosted | docs-only | LangChain's SDK for agent long-term memory — semantic, episodic, and procedural memory over LangGraph's store, with prompt optimization. |
| [MemoryOS](#memoryos) | 4.3 Organizational / semantic memory | Apache-2.0 | 1,517 | Emerging | 🟢 Active | 🟡 partial | docs-only | Academic memory operating system for personalized agents — OS-inspired 3-tier hierarchy (short/mid/long-term) with FIFO + paging updates. EMNLP 2025 Oral. |
| [A-MEM](#a-mem) | 4.3 Organizational / semantic memory | MIT | 1,116 | Emerging | ⚠️ Caution | 🟡 partial | docs-only | Zettelkasten-inspired agentic memory — note-based memory that dynamically links notes and evolves older ones as new memories arrive. |
| [Redis Agent Memory Server](#redis-agent-memory-server) | 4.3 Organizational / semantic memory | Redis Source Available (RSALv2/SSPLv1) / NOASSERTION | 295 | Emerging | 🟢 Active | 🟡 partial | docs-only | Official Redis reference agent-memory server — two-tier working + long-term memory, REST + MCP, Redis-backed semantic search, auto extract/dedup/forget. |
| [SynaBun](#synabun) | 4.3 Organizational / semantic memory | Apache-2.0 | 68 | Preview | 🟢 Active | ✅ | docs-only | Local-first coding-agent memory (part of a vibe coding toolkit) — SQLite + embedded models, no external APIs, MCP + Claude Code hooks, 3D memory-graph viz. |
...
| [Letta](#letta) | 4.3 Organizational / semantic memory | Apache-2.0 / NOASSERTION | — | Established | ⚠️ Caution | ✅ | docs-only | Framework for stateful agents with persistent, self-improving memory (editable memory blocks) plus background sleep-time compute; formerly MemGPT. |
| [OWASP Agent Memory Guard](#owasp-agent-memory-guard) | 4.4 Memory security & governance | Apache-2.0 | 97 | Preview | 🟢 Active | ✅ | docs-only | OWASP reference runtime defense for memory poisoning (ASI06) — a guard layer between agent and memory store, screening every read/write via detectors + YAML policy. |
...
| [Cipher](#cipher) | You want to give coding agents persistent, cross-IDE, team-shareable memory of concepts + reasoning (MCP). | You need general (non-coding) memory (Mem0), or a clean OSI license today (NOASSERTION). |
...
| License | Apache-2.0 |
| Maturity | Prominent · ⭐87,904 · created 2025-08 |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| Sub-type | 4.1 Project / task memory |
| License | MIT |
| Maturity | Prominent · ⭐61,125 · created 2026-01 · TypeScript |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| Sub-type | 4.1 Project / task memory |
| License | MIT |
| Maturity | Prominent · ⭐57,490 · created 2026-04 · Python |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| Sub-type | 4.1 Project / task memory |
| License | MIT |
| Maturity | Prominent · ⭐33,032 · created 2026-02 · C |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| Maturity | Prominent · ⭐27,870 · created 2025-03 |
| Runs local | 🟡 partial |
| Hands-on | docs-only |
...
| Sub-type | 4.1 Project / task memory |
| License | MIT |
| Maturity | Prominent · ⭐25,436 · created 2025-10 · Go |
| Runs local | ✅ |
| Hands-on | run |
...
| Sub-type | 4.1 Project / task memory |
| License | Apache-2.0 |
| Maturity | Emerging · ⭐1,678 · created 2025-02 |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| Sub-type | 4.1 Project / task memory |
| License | MIT |
| Maturity | Emerging · ⭐1,404 · created 2026-03 · TypeScript |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| License | Apache-2.0 |
| Maturity | Prominent · ⭐61,238 · created 2023-06 · Python |
| Runs local | 🟡 partial |
| Hands-on | inspected |
...
| Sub-type | 4.2 Personal memory |
| License | MIT |
| Maturity | Prominent · ⭐28,490 · created 2024-02 · TypeScript |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| Maturity | Prominent · ⭐15,633 · created 2025-07 · Python |
| Runs local | 🟡 partial |
| Hands-on | docs-only |
...
| Sub-type | 4.2 Personal memory |
| License | Apache-2.0 |
| Maturity | Established · ⭐4,355 · created 2025-10 |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| License | Apache-2.0 |
| Maturity | Prominent · ⭐28,940 · created 2024-08 · Python |
| Runs local | ☁️ hosted |
| Hands-on | docs-only |
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | Apache-2.0 |
| Maturity | Prominent · ⭐28,535 · created 2023-08 · Python |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | MIT |
| Maturity | Prominent · ⭐26,622 · created 2026-04 |
| Runs local | 🟡 partial |
| Hands-on | docs-only |
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | NOASSERTION |
| Maturity | Prominent · ⭐14,048 · created 2025-07 · Python |
| Runs local | 🟡 partial |
| Hands-on | docs-only |
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | MIT |
| Maturity | Established · ⭐5,654 · created 2026-04 · Python |
| Runs local | ✅ |
| Hands-on | docs-only |
...
**🟢 Active** — Open-source memory layer for coding agents — dual System-1/System-2 memory, cross-IDE via MCP, team-shareable; renamed to ByteRover CLI (brv).
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | NOASSERTION |
| Maturity | Established · ⭐4,925 · created 2025-06 · TypeScript |
| Runs local | 🟡 partial |
| Hands-on | docs-only |
...
**Avoid when** — You need general (non-coding) memory (Mem0), or a clean OSI license today (NOASSERTION).
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | Apache-2.0 |
| Maturity | Established · ⭐4,766 · created 2023-04 |
| Runs local | ☁️ hosted |
| Hands-on | docs-only |
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | MIT |
| Maturity | Established · ⭐1,568 · created 2025-01 · Python |
| Runs local | ☁️ hosted |
| Hands-on | docs-only |
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | Apache-2.0 |
| Maturity | Emerging · ⭐1,517 · created 2025-05 · Python |
| Runs local | 🟡 partial |
| Hands-on | docs-only |
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | MIT |
| Maturity | Emerging · ⭐1,116 · created 2025-02 · Python |
| Runs local | 🟡 partial |
| Hands-on | docs-only |
...
| Maturity | Emerging · ⭐295 · created 2025-03 · Python |
| Runs local | 🟡 partial |
| Hands-on | docs-only |
...
| Sub-type | 4.3 Organizational / semantic memory |
| License | Apache-2.0 |
| Maturity | Preview · ⭐68 · created 2026-02 |
| Runs local | ✅ |
| Hands-on | docs-only |
...
**⚠️ Caution** — Framework for stateful agents with persistent, self-improving memory (editable memory blocks) plus background sleep-time compute; formerly MemGPT.
...
| Maturity | Established · created 2023 |
| Runs local | ✅ |
| Hands-on | docs-only |
...
| License | Apache-2.0 |
| Maturity | Preview · ⭐97 · created 2026-02 · Python |
| Runs local | ✅ |
| Hands-on | docs-only |
